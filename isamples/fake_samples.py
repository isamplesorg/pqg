...
def fake_GeospatialCoordLocations(
    count: int = 1, base: int = 0
) -> typing.Generator[GeospatialCoordLocation, None, None]:
    for i in range(count):
...
def fake_IdentifiedConcept(domain: str) -> IdentifiedConcept:
    ...
    return IdentifiedConcept(
        pid=f"{uri}{label}", label=generator.word(), scheme_name=domain, scheme_uri=uri
    )


def fake_IdentifiedConcepts(
    domain: str, count: int = 1
) -> typing.Generator[IdentifiedConcept, None, None]:
    for i in range(count):
...
def fake_SampleRelations(
    target: str, count: int = 1, base: int = 0
) -> typing.Generator[SampleRelation, None, None]:
    for i in range(count):
...
def fake_MaterialSampleCuration(
    i: int, responsibility: typing.Optional[typing.List[Agent]] = None
) -> MaterialSampleCuration:
...
def make_fakes(g: pqg.PQG, count: int = 5):
    ...
    context_categories = list(
        fake_IdentifiedConcepts("context_category", count=count * 5)
    )
    material_categories = list(
        fake_IdentifiedConcepts("material_category", count=count * 5)
    )
    sample_object_types = list(
        fake_IdentifiedConcepts("sample_object_type", count=count * 5)
    )
    ...
    for i in range(count):
        ...
        ms = fake_MaterialSampleRecord(
            ...
            has_material_category=list(
                generator.random_choices(material_categories, 2)
            ),
            has_sample_object_type=list(
                generator.random_choices(sample_object_types, 2)
            ),
        )
        g.addNode(ms)
